...
def apply_flex_dispatcher_backend(
    ...
    if model_config.num_moe_experts is None or model_config.num_moe_experts == 0:
        if get_rank_safe() == 0:
            logger.warning(
                "DeepEP or HybridEP is only applicable to MoE models. "
    ...
    if moe_flex_dispatcher_backend == "deepep":
        if torch.cuda.get_device_properties(0).major not in [8, 9]:
            if get_rank_safe() == 0:
                logger.warning(
                    "DeepEP is only applicable to Ampere (SM80) and Hopper (SM90) GPUs. "
                    "Skipping DeepEP configuration."
                )
            return
    elif moe_flex_dispatcher_backend == "hybridep":
        if not (torch.cuda.get_device_properties(0).major == 10 and
            torch.cuda.get_device_properties(0).name in ["NVIDIA GB200", "NVIDIA GB300"]):
            if get_rank_safe() == 0:
                logger.warning(
                    "HybridEP is only applicable to GB200 GPUs with NVL72. "
                    "Skipping HybridEP configuration."
                )
            return
    else:
        if get_rank_safe() == 0:
            logger.warning(
                "Not a valid flex dispatcher backend. "
...
def validate_flex_dispatcher_backend(model_config: TransformerConfig) -> None:
    ...
    if model_config.moe_token_dispatcher_type == "flex":
        if (model_config.moe_flex_dispatcher_backend == "deepep" and 
            torch.cuda.get_device_properties(0).major not in (8, 9)):
            raise ValueError("DeepEP is supported for Ampere (SM80) and Hopper (SM90) GPUs")

        if (model_config.moe_flex_dispatcher_backend == "hybridep" and 
            not (torch.cuda.get_device_properties(0).major == 10 and
            torch.cuda.get_device_properties(0).name in ["NVIDIA GB200", "NVIDIA GB300"])):
